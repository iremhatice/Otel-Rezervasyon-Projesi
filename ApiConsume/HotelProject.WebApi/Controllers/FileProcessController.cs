...
namespace HotelProject.WebApi.Controllers
{
    ...
    {
        [HttpPost]
        public async Task<IActionResult> UploadFile([FromForm] IFormFile file) //Dosya Yükleme İşlemi,Dosyadan çekildiği için FromForm kullanıldı.
        {
            var fileName = Guid.NewGuid() + Path.GetExtension(file.FileName); //GetExtension ile uzantı alınır.
            var path = Path.Combine(Directory.GetCurrentDirectory(), "files/" + fileName); //Verilen yola kaydeder.
